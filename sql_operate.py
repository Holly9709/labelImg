...
def insert2sql(xmlfile):
    ...
    try:
        ...
        print e
        if e[0] == 1406:     
            cursor.execute("ALTER TABLE %sxml MODIFY xml_content VARCHAR(5000)"%project_name)
            print ("ALTER TABLE %sxml MODIFY xml_content VARCHAR(5000)"%project_name)
        cursor.execute(sql)
...
def imagefromsql(dirpath, softwareMode = 'resume'):
    ...
    if softwareMode == 'verify':
        ...
        if remainNum == 0:
            print "All images have been verified."
            return
    ...
    #This is browse mode, which can get images from id == 1.
    elif softwareMode == 'no_resume':
        
        cursor = cursor_execute("SELECT id FROM %simg ORDER BY id DESC LIMIT 1"%(project_name))
        lastImgNum = cursor[0]
        if lastImgNum < idNum:
